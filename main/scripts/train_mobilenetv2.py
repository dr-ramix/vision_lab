...
if cfg.use_class_weights:
    ...
    (out_dir / "class_weights.json").write_text(
        json.dumps(w.detach().cpu().tolist(), indent=2)
    )
else:
...
for epoch in range(1, cfg.epochs + 1):
    train_loss = train_one_epoch(
        ...
        use_amp, cfg.grad_clip
    )
    scheduler.step()

    val_loss, val_metrics, val_cm = evaluate(
    ...
    (out_dir / f"val_metrics_epoch_{epoch:03d}.json").write_text(
    ...
    if cfg.early_stop_patience > 0 and patience_left <= 0:
        print(
            ...
            f"best f1_macro: {best_val_f1:.4f}"
        )
        break
...
for epoch in range(1, cfg.epochs + 1):
    ...
    print(
        f"[FT] Epoch {epoch:02d}/{cfg.epochs} | "
        f"train {train_loss:.4f} | "
        f"val {val_loss:.4f} | "
        f"acc {val_acc:.4f} | "
        f"f1_macro {val_f1:.4f}"
