...
# ============================================================
def _resolve_mtcnn_cropped_int_norm_paths(standardized_root: Path) -> Tuple[Path, Path]:
    """
    User contract:
    ...
    We must find:
      npy_root   = standardized_root/mtcnn_cropped_int_norm/npy
      stats_json = standardized_root/mtcnn_cropped_int_norm/dataset_stats_train.json

    Also supports the case where someone accidentally passes already the npy_root
    or already the mtcnn_cropped_int_norm root.
    """
    standardized_root = Path(standardized_root)

    # case A: caller already passes ".../mtcnn_cropped_int_norm/npy"
    if standardized_root.name == "npy" and standardized_root.parent.name == "mtcnn_cropped_int_norm":
        npy_root = standardized_root
        stats_json = standardized_root.parent / "dataset_stats_train.json"
        return npy_root, stats_json

    # case B: caller passes ".../mtcnn_cropped_int_norm"
    if standardized_root.name == "mtcnn_cropped_int_norm":
...
class NpyFolderDataset(Dataset):
    ...
    def _to_chw_float01(x: np.ndarray) -> torch.Tensor:
        ...
        # If single-channel, repeat to 3 (stats are 3-channel)
        if xt.shape[0] == 1:
            xt = xt.repeat(3, 1, 1)
...
def build_dataloaders(
    *,
    images_root: Path,              # expected: .../standardized (or .../mtcnn_cropped_int_norm[/npy])
    batch_size: int = 64,
    num_workers: int = 4,
    ...
    actual data:
      .../standardized/mtcnn_cropped_int_norm/npy/{train,val,test}/{class}/*.npy
    ...
    npy_root, default_stats = _resolve_mtcnn_cropped_int_norm_paths(images_root)
