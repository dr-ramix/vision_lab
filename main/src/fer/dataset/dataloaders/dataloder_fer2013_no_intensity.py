...
def _resolve_mtcnn_cropped_paths(standardized_root: Path) -> Tuple[Path, Path]:
    ...
    if standardized_root.name == "npy" and standardized_root.parent.name == "fer2013_mtcnn_cropped":
    ...
    # case B: caller passes ".../fer2013_mtcnn_cropped"
    if standardized_root.name == "fer2013_mtcnn_cropped":
        npy_root = standardized_root / "npy"
        stats_json = standardized_root / "dataset_stats_train.json"
        return npy_root, stats_json

    # case C: caller passes ".../standardized"
    fer2013_root = standardized_root / "fer2013" / "fer2013_mtcnn_cropped"
...
def build_dataloaders(
    *,
    ...
    npy_root, default_stats = _resolve_mtcnn_cropped_paths(images_root)
