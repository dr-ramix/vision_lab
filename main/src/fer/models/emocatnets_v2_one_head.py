...
class EmoCatNetV2OneHeadConfig:
...
EMOCATNETV2ONEHEAD_SIZES: Dict[str, EmoCatNetV2OneHeadConfig] = {
    "nano": EmoCatNetV2OneHeadConfig(
        depths=(3, 3, 6, 2),
        dims=(64, 128, 256, 512),
        drop_path_rate=0.06,
        num_heads=8,
    ...
    "small": EmoCatNetV2OneHeadConfig(
        depths=(3, 3, 15, 2),
        dims=(96, 192, 384, 768),
        drop_path_rate=0.15,
        num_heads=8,
    ...
    "large": EmoCatNetV2OneHeadConfig(
        depths=(3, 3, 27, 2),
    ...
    "xlarge": EmoCatNetV2OneHeadConfig(
        depths=(3, 3, 27, 2),
...
# ============================================================
# Model: EmoCatNetV2OneHead
# ============================================================

class EmoCatNetV2OneHead(nn.Module):
    """
    Pipeline:
      (optional) STN -> stem(64->64)
      -> stage1 + cbam1
      -> down1 -> stage2 + cbam2
      -> down2 -> stage3 + cbam3
      -> down3 -> stage4(tokens) -> cbam4
      -> GAP(8x8) -> LN(d3) -> head
    """
    def __init__(
        self,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feat = x.mean(dim=(-2, -1))  # GAP 8x8 -> (B, d3)
        feat = self.final_ln(feat)
        return self.head(feat)


def emocatnetv2onehead_fer(
    size: str = "tiny",
    in_channels: int = 3,
    num_classes: int = 6,
    *,
    ...
    size = size.lower()
    if size not in EMOCATNETV2ONEHEAD_SIZES:
        raise ValueError(f"Unknown size='{size}'. Valid: {list(EMOCATNETV2ONEHEAD_SIZES.keys())}")

    cfg = EMOCATNETV2ONEHEAD_SIZES[size]
    return EmoCatNetV2OneHead(
...
def _shape_test(image_size: int = 64, batch_size: int = 2, num_classes: int = 6, in_channels: int = 3):
    ...
    for name in EMOCATNETV2ONEHEAD_SIZES.keys():
        model = emocatnetv2onehead_fer(name, in_channels=in_channels, num_classes=num_classes, use_stn=True).to(device).eval()
