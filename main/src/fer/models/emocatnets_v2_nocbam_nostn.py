"""
EmoCatNets-v2 (64x64 FER) — NO STN + stem (NO residual STN, NO residual stem)
Variant: NO CBAM after transformer stage4

This new model is: EmoCatNetsV2NoCBAMNoSTN
"""
...
class EmoCatNetsV2NoCBAMNoSTN(nn.Module):
    """
...
def emocatnetsv2_nocbam_nostn_fer(
    size: str = "tiny",
    in_channels: int = 3,
    num_classes: int = 6,
    *,
    ...
    return EmoCatNetsV2NoCBAMNoSTN(
...
def _shape_test(image_size: int = 64, batch_size: int = 2, num_classes: int = 6, in_channels: int = 3):
    ...
    for name in EMOCATNETS_V2_SIZES.keys():
        model = emocatnetsv2_nocbam_nostn_fer(name, in_channels=in_channels, num_classes=num_classes).to(device).eval()
