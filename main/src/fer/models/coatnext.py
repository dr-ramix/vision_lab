...
- Down1: LN + Conv(k2,s2) 64->32
- Stage2: ConvNeXt blocks @32x32
- Down2: LN + Conv(k2,s2) 32->16
- Stage3: ConvNeXt blocks @16x16
- Down3: LN + Conv(k2,s2) 16->8
- Stage4: Relative Transformer blocks on 8x8 tokens (64 tokens)
- Head: GAP -> LN -> Linear
...
class CoAtNeXt(nn.Module):
    ...
    def __init__(
        ...
        attn_dropout: float = 0.0,
        proj_dropout: float = 0.0,
        token_hw: Tuple[int, int] = (8, 8),  # (H,W) before transformer for 64x64 with this plan
    ):
        super().__init__()
...
def _shape_test(image_size: int = 64, batch_size: int = 2, num_classes: int = 6, in_channels: int = 3):
    ...
    for name in COATNEXT_SIZES.keys():
        model = coatnext_fer(name, in_channels=in_channels, num_classes=num_classes).to(device).eval()
