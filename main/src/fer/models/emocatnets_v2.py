...
# (fixed grid size per stage init)
# ============================================================
...
# ============================================================
# "Better attention": Conv Positional Encoding (CPE)
# ============================================================
...
def _shape_test(image_size: int = 64, batch_size: int = 2, num_classes: int = 6, in_channels: int = 3):
    ...
    for name in EMOCATNETS_V2_SIZES.keys():
        model = emocatnets_v2_fer(name, in_channels=in_channels, num_classes=num_classes).to(device).eval()
...
def _one_train_step(size: str = "tiny", image_size: int = 64, num_classes: int = 6, in_channels: int = 3):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = emocatnets_v2_fer(size=size, in_channels=in_channels, num_classes=num_classes).to(device).train()
