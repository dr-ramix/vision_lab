...
# ============================================================
# Config / Sizes (renamed)
# ============================================================

@dataclass(frozen=True)
class EmoCatNetV2ResidualConfig:
...
EMOCATNETS_V2_RESIDUAL_SIZES: Dict[str, EmoCatNetV2ResidualConfig] = {
    "nano": EmoCatNetV2ResidualConfig(
        depths=(3, 3, 6, 2),
        dims=(64, 128, 256, 512),
        drop_path_rate=0.06,
        num_heads=8,
    ...
    "tiny": EmoCatNetV2ResidualConfig(
        depths=(3, 3, 9, 2),
        dims=(96, 192, 384, 768),
        drop_path_rate=0.10,
        num_heads=8,
    ...
    "small": EmoCatNetV2ResidualConfig(
        depths=(3, 3, 15, 2),
        dims=(96, 192, 384, 768),
        drop_path_rate=0.15,
        num_heads=8,
    ...
    "base": EmoCatNetV2ResidualConfig(
        depths=(3, 3, 18, 2),
    ...
    "large": EmoCatNetV2ResidualConfig(
        depths=(3, 3, 27, 2),
    ...
    "xlarge": EmoCatNetV2ResidualConfig(
        depths=(3, 3, 27, 2),
...
class EmoCatNetsV2Residual(nn.Module):
    """
    Same as EmoCatNetsV2, renamed for separate registry entry.
    """
    def __init__(
        self,
        in_channels: int = 3,
...
def emocatnetsv2residual_fer(
    ...
    *,
    ...
    stn_hidden: int = 32,
    stn_alpha_init: float = 0.15,
) -> EmoCatNetsV2Residual:
    size = size.lower()
    if size not in EMOCATNETS_V2_RESIDUAL_SIZES:
        raise ValueError(f"Unknown size='{size}'. Valid: {list(EMOCATNETS_V2_RESIDUAL_SIZES.keys())}")

    cfg = EMOCATNETS_V2_RESIDUAL_SIZES[size]
    return EmoCatNetsV2Residual(
