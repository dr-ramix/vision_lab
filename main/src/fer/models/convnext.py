...
class ConvNextBlock(nn.Module):
    ...
    def __init__(self, dim: int, drop_path: float = 0.0, layer_scale_init_value: float = 1e-6):
        ...
        self.layer_norm = nn.LayerNorm(dim, eps=1e-6)  # used on NHWC
...
class ConvNeXtFER(nn.Module):
    """
    ConvNeXt-based CNN for small images.

    stem -> stage1 -> down1 -> stage2 -> down2 -> stage3 -> down3 -> stage4 -> GAP -> LN -> head
...
def _shape_test(image_size: int = 64, batch_size: int = 2, num_classes: int = 6):
    ...
    for name in CONVNEXTFER_SIZES.keys():
        model = convnextfer(name, num_classes=num_classes).to(device).eval()
...
def _one_train_step(size: str = "tiny", image_size: int = 64, num_classes: int = 6):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = convnextfer(size, num_classes=num_classes).to(device).train()
...
if __name__ == "__main__":
    ...
    _one_train_step(size="small", image_size=64, num_classes=6)